...
func registerMiscRoutes(mux *http.ServeMux, eng *engine.Engine, subMgr *subscription.Manager, updateChecker *update.Checker) {
	...
	mux.HandleFunc("GET /api/logs/export", func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte("  State: " + status.State + "\n"))
		_, _ = w.Write([]byte("  Transport: " + status.Transport + "\n"))
		_, _ = w.Write([]byte("  Active Connections: " + fmt.Sprintf("%d", status.ActiveConns) + "\n"))
		_, _ = w.Write([]byte("  Total Connections: " + fmt.Sprintf("%d", status.TotalConns) + "\n"))
		_, _ = w.Write([]byte("  Bytes Sent: " + fmt.Sprintf("%d", status.BytesSent) + "\n"))
		_, _ = w.Write([]byte("  Bytes Received: " + fmt.Sprintf("%d", status.BytesReceived) + "\n\n"))

		_, _ = w.Write([]byte("Configuration:\n"))
		_, _ = w.Write([]byte("  Server: " + cfg.Server.Addr + "\n"))
		_, _ = w.Write([]byte("  Log Level: " + cfg.Log.Level + "\n"))
		_, _ = w.Write([]byte("  SOCKS5: " + cfg.Proxy.SOCKS5.Listen + "\n"))
		_, _ = w.Write([]byte("  HTTP: " + cfg.Proxy.HTTP.Listen + "\n"))
	})
