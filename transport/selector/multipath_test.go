...
func TestMultipathPoolVirtualConnOpenStream(t *testing.T) {
	...
	pool := NewMultipathPool(ctx, []transport.ClientTransport{ft}, "localhost:443", NewMinLatencyScheduler(), 0, 0, nil)
	defer pool.Close()
...
func TestMultipathPoolFallbackOnStreamError(t *testing.T) {
	...
	pool := NewMultipathPool(ctx, []transport.ClientTransport{ft1, ft2}, "localhost:443", NewMinLatencyScheduler(), 0, 0, nil)
	defer pool.Close()
